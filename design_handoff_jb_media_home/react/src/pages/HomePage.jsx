...
const MARQUEE = ["Connect", "Communicate", "Collaborate"];
...
export function HomePage() {
  ...
  return (
    <div className="flex flex-col min-h-screen">
      {/* ---------------- HERO SECTION ---------------- */}
      <section ref={heroRef} className="relative min-h-screen flex flex-col justify-center overflow-hidden pt-24 pb-16">
        ...
        {/* Hero Content Grid */}
        <div className="relative z-10 max-w-7xl mx-auto px-6 sm:px-8 w-full grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-6 items-center">
          {/* Left Column Copy matching reference picture */}
          <div className="lg:col-span-6 flex flex-col gap-6 text-left z-20">
            <div className="flex items-center gap-3">
              <Badge variant="default" className="gap-1.5 px-3.5 py-1">
                <Sparkles className="w-3.5 h-3.5 text-gold-300" />
                <span>The Official Media Club</span>
              </Badge>
            ...
            <h1 className="font-anton text-6xl sm:text-7xl lg:text-8xl xl:text-9xl tracking-tight uppercase leading-[0.88] text-foreground">
              INFINITE <br />
              <span className="bg-gold-gradient bg-clip-text text-transparent">ARCHIVE</span>
            </h1>

            <p className="font-barlow text-lg sm:text-xl text-foreground/80 max-w-lg leading-relaxed font-light">
              A Global, Infinite Radial 3D Sphere chronicling the life, pulse, and legacy of JBIET. Drag to spin, scroll to zoom across our college history.
            </p>
      ...
      {/* ---------------- MARQUEE TICKER ---------------- */}
      <section className="jb-marquee">
        <div className="jb-marquee__track">
          {[0, 1, 2].map((pass) =>
            MARQUEE.map((word, i) => (
              <span key={`${pass}-${word}`} className="contents">
